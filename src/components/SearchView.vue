<template>
    <div class="search">
        ...
        <input type="text" v-model="searchTerm" @input="handleSearch" placeholder="Search..." class="search-home" >
       ...
       <ul v-if="showResults" class="search-results">
           <li v-for="(result, index) in searchResults" :key="index" class="search-result">
               <button @click="this.$router.push({ name: 'documents', params: { id: result.id } })" class="file-button">
                   <i class="bi bi-file-earmark-pdf"></i>
               </button>
               <div class="result-details"  @click="this.$router.push({ name: 'documents', params: { id: result.id } })">
        ...
        const handleSearch = () => {
            ...
            const filteredResults = files.value.filter((item) => {
                ...
                for (const value of values) {
                    if (value && typeof value === 'string' && value.toLowerCase().includes(searchTerm.value.toLowerCase())) {
