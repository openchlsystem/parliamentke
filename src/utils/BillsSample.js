...
// Iterate through the BillsSample array and update date formats
const updatedBillsSample = BillsSample.map(bill => {
  const updatedBill = { ...bill }; // Creating a copy of the object to avoid mutation

  if (updatedBill.date) {
    updatedBill.date = convertDateFormat(updatedBill.date);
  }

  if (updatedBill.maturity_date) {
    updatedBill.maturity_date = convertDateFormat(updatedBill.maturity_date);
  }

  if (updatedBill["1ST  READ"] !== undefined && typeof updatedBill["1ST  READ"] === 'string') {
    updatedBill["1ST  READ"] = convertDateFormat(updatedBill["1ST  READ"]);
  }

  return updatedBill;
});

export { updatedBillsSample };
